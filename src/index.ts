...
Keep the structure flat for depth ${depth}, focusing on main sections only.`
  ...
  const languageModel = provider.languageModel(
    model.startsWith('@cf/') ? model : DEFAULT_MODEL,
    {
...
async function generateContentForOutline(outline: OutlineItem[], type: string, model: string): Promise<string> {
...
Ensure all JSX/MDX syntax is valid and can be parsed by the MDX compiler.`
  ...
  try {
    ...
    const rawContent = generatedText.trim().startsWith('---') ? generatedText : `---
$schema: https://mdx.org.ai/schema.json
$type: ${type}
model: ${model}
---
